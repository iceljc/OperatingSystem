...
/* What to do when timer interrupt occurs? In this case, we update "ticks",
   ...
   when the system gets initialized. (e.g. in "kernel.C") */
    ...
    if (ticks >= (hz/20))
    {
        ...
        Console::puts(" Round Robin switch...\n");
...
void RRTimer::set_frequency(int _hz) {
...
void RRTimer::current(unsigned long * _seconds, int * _ticks) {
...
void RRTimer::wait(unsigned long _seconds) {
